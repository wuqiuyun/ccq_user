...
public class Tools_Activity extends Activity implements OnClickListener {
	...
	private void convert() {
		try {
			...
			/**
			 * 设置坐标来源,这里使用百度坐标作为示例 可选的来源包括： <li>CoordType.BAIDU ： 百度坐标 <li>
			 * CoordType.MAPBAR ： 图吧坐标 <li>CoordType.MAPABC ： 图盟坐标 <li>
			 * CoordType.SOSOMAP ： 搜搜坐标 <li>CoordType.ALIYUN ： 阿里云坐标 <li>
			 * CoordType.GOOGLE ： 谷歌坐标 <li>CoordType.GPS ： GPS坐标
			 */
			...
			if (null != destPoint) {
				tvConvertReult.setText("转换后坐标(经度、纬度):"
						+ destPoint.getLongitude() + ","
						+ destPoint.getLatitude());
			} else {
				Toast.makeText(getApplicationContext(), "坐标转换失败",
	...
	private void checkIsAvaliable() {
		...
		CoordinateConverter converter = new CoordinateConverter(
				getApplicationContext());
		// 判断是否高德地图可用的坐标
		boolean result = converter.isAMapDataAvailable(
				examplePoint.getLatitude(), examplePoint.getLongitude());
		if (result) {
