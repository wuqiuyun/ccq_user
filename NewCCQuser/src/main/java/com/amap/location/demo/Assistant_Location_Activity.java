...
public class Assistant_Location_Activity extends CheckPermissionsActivity
	...
	public void onClick(View v) {
		if (v.getId() == R.id.bt_assistant) {
			if (btAssistant.getText().equals(
					getResources().getString(R.string.startAssistantLocation))) {
				btAssistant.setText(getResources().getString(
						R.string.stopAssistantLocation));
				locationClient.startAssistantLocation();
				btLocation.setEnabled(true);
			} else {
				btAssistant.setText(getResources().getString(
						R.string.startAssistantLocation));
				locationClient.stopAssistantLocation();
			...
			// 浏览器定位，加载预置的H5定位页面
			webView.loadUrl(LocationUtils.URL_H5LOCATION);
		}
	}
}
