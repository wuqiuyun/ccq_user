 ...
 * 
 */
public class LoginMiMaActivity extends BaseActivity implements OnClickListener {
	...
	private Handler handler = new Handler() {
		public void handleMessage(android.os.Message msg) {
			...
			case HANDLER_NET_FAILURE:
				dissDialog();
				Toast.makeText(LoginMiMaActivity.this, "当前网络不可用,请检查网络",
						Toast.LENGTH_SHORT).show();
				break;
			// 错误
			case HANDLER_NN_FAILURE:
				dissDialog();
				Toast.makeText(LoginMiMaActivity.this, "当前网络出错,请检查网络",
						Toast.LENGTH_SHORT).show();
				break;
			// 获取信息失败
			case HANDLER_GETINFO_FAILURE:
				dissDialog();
				Toast.makeText(LoginMiMaActivity.this,
						msg.obj.toString().trim(), Toast.LENGTH_SHORT).show();
				break;
			// 获取我的信息成功
			case HANDLER_CHECKMIMA_SUCCESS:
				dissDialog();
				Intent intent = new Intent(LoginMiMaActivity.this,
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.activity_loginmima);
	...
	private void checkMiMaHttpPost() {
		...
		String[] verstring = utils.getVersionInfo(LoginMiMaActivity.this);

		RequestParams params = new RequestParams();
		params.addHeader("ccq", utils.getSystemVersion() + "," + verstring[0]
				+ "," + verstring[2]);
		params.addBodyParameter("token",
				new DButil().gettoken(LoginMiMaActivity.this));
		params.addBodyParameter("member_id",
				new DButil().getMember_id(LoginMiMaActivity.this));
	...
	private boolean canPost() {
		...
		if (TextUtils.isEmpty(mima)) {
			Toast.makeText(LoginMiMaActivity.this, "原密码不能为空",
					Toast.LENGTH_SHORT).show();
			return false;
		} else if (mima.length() < 6) {
			Toast.makeText(LoginMiMaActivity.this, "原密码不能少于6位",
					Toast.LENGTH_SHORT).show();
			return false;
		}
		return true;
	...
	private void showLoading() {
		View view = LayoutInflater.from(LoginMiMaActivity.this).inflate(
				R.layout.loading, null);
		ImageView image = (ImageView) view.findViewById(R.id.iv_loading);
		Animation animation = AnimationUtils.loadAnimation(
				LoginMiMaActivity.this, R.anim.loading_anim);
		image.startAnimation(animation);
		loadingDialog = new Dialog(LoginMiMaActivity.this, R.style.mDialogStyle);
